...
const IncidentsTable = ({ activeView, onViewChange }: IncidentsTableProps) => {
  ...
  // Read URL parameters and apply participant filter
  useEffect(() => {
  ...
  // Reload statistics and data when URL participant filter is applied
  useEffect(() => {
    if (selectedParticipant !== 'all' && location.search.includes('participant_id')) {
      // Trigger refresh to apply the filter
      setRefreshKey((prev) => prev + 1);

      // Reload statistics with participant filter
  ...
  }, [selectedParticipant, location.search]);
  ...
  const handleToggleAll = () => {
    const sectionKeys = ['basicInfo', 'whatHappened', 'leadUpTriggers', 'duringIncident', 'responseActions', 'causesFactors'];
    // If any section is open (false), we should close all. If all are closed, we should open all.
    // Actually, user's request "toggle or sum button which open or close all" 
    // Usually means if any are collapsed, expand all, otherwise collapse all.
    const anyCollapsed = sectionKeys.some(key => collapsedSections[key]);

    const newState: Record<string, boolean> = {};
    sectionKeys.forEach(key => {
      newState[key] = !anyCollapsed; // If any collapsed, set all to false (expanded). Else set all to true.
    });
  ...
  const closeReportModal = () => {
    ...
    // Reset collapsed sections - all open by default
  ...
  const fetchIncidentsData = async (params: TDataGridRequestParams) => {
    try {
      ...
      if (selectedParticipant !== 'all') {
        filters.participant_id = parseInt(selectedParticipant);
      }
      ...
      console.log('🔍 DEBUG - First incident sample:', incidentsData[0]);
      console.log('🔍 DEBUG - Incident IDs:', incidentsData.map((inc: any) => inc.id));
  ...
  const handleRowSelection = (state: RowSelectionState) => {
    console.log('🔍 DEBUG - Row selection changed:', state);
  ...
  const handleBulkDelete = async () => {
    ...
    console.log('🔍 DEBUG - Selected rows state:', selectedRows);
    console.log('🔍 DEBUG - Selected IDs from keys:', selectedIds);
    console.log('🔍 DEBUG - Converting to numbers:', selectedIds.map(id => Number(id)));
    ...
    try {
      ...
      const deletePromises = selectedIds.map(id => {
        ...
        console.log('🔍 DEBUG - Deleting incident with ID:', numId);
    ...
    try {
      ...
      if (selectedParticipant && selectedParticipant !== 'all') {
        dataToExport = dataToExport.filter((item: any) => {
          const participantId = String(item.participant_id || item.customer?.id || '');
          return participantId === String(selectedParticipant);
        });
      }
  ...
  return (
    <>
      <DataGrid
        ...
        rowSelection={true}
        getRowId={(row: any) => {
          console.log('🔍 DEBUG - getRowId called with row:', row);
          console.log('🔍 DEBUG - Returning ID:', String(row.id));
            ...
            <div className="modal-content" style={{ display: 'flex', flexDirection: 'column', height: '100%', minWidth: 0, width: '100%' }}>
              ...
              <div className="modal-header" style={{ padding: '20px 24px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <span className="badge badge-outline badge-success rounded-full" style={{ fontSize: '0.75rem', padding: '4px 8px' }}>
                  ...
                  <div style={{
                              ...
                              {bspAnalysisData.bsp_analysis?.gaps_detected?.count > 0 && (
                                <div style={{
                                  ...
                                  >
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                      <span className="badge badge-outline badge-danger rounded-full" style={{ fontSize: '0.6875rem', padding: '4px 10px', fontWeight: '600' }}>
                              ...
                              {bspAnalysisData.bsp_analysis.draft_update?.data && (
                                <div style={{
                                  ...
                                  >
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                      <span className="badge badge-outline badge-primary rounded-full" style={{ fontSize: '0.6875rem', padding: '4px 10px', fontWeight: '600' }}>
                                        Draft
                                      </span>
                              ...
                              {bspAnalysisData.bsp_analysis.trigger_alignment?.data && (
                                <div style={{
                                  ...
                                  >
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                      <span className="badge badge-outline badge-info rounded-full" style={{ fontSize: '0.6875rem', padding: '4px 10px', fontWeight: '600' }}>
                                    ...
                                    <div style={{ padding: '16px', backgroundColor: '#f5f3ff' }}>
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                        {(bspAnalysisData.bsp_analysis.skill_building.data || []).map((item: any, idx: number) => (
                                          <div key={idx} style={{ backgroundColor: 'white', padding: '12px', borderRadius: '6px', border: '1px solid #e9d5ff' }}>
                                            {item.skill_area && (
                                              <span className="badge badge-outline badge-sm badge-light rounded-full" style={{ fontSize: '0.75rem', padding: '4px 8px', marginBottom: '8px', display: 'inline-block' }}>
                                                {item.skill_area}
                                              </span>
                                            )}
                                            {item.recommendation && (
                              ...
                              {bspAnalysisData.bsp_analysis.risk_insights?.data && (
                                <div style={{
                                  ...
                                  >
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                      <span className={`badge badge-outline rounded-full ${bspAnalysisData.bsp_analysis.risk_insights.data.recurrence_risk === 'high' ? 'badge-danger' :
                              ...
                              {bspAnalysisData.bsp_analysis.compliance?.data && (
                                <div style={{
                                  ...
                                  >
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                      <span className={`badge badge-outline rounded-full ${bspAnalysisData.bsp_analysis.compliance.data.compliance_level === 'compliant' ? 'badge-success' :
                                      ...
                                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                                        {bspAnalysisData.bsp_analysis.compliance.data.person_centred_practice && (
                                          <div style={{ backgroundColor: 'white', padding: '12px', borderRadius: '6px', border: '1px solid #e9d5ff' }}>
                                            ...
                                            <div className="flex items-center justify-between">
                                              <span className={`badge badge-outline rounded-full ${bspAnalysisData.bsp_analysis.compliance.data.person_centred_practice.status === 'compliant' ? 'badge-success' : 'badge-danger'}`} style={{ fontSize: '0.75rem', padding: '4px 8px' }}>
                                        ...
                                        {bspAnalysisData.bsp_analysis.compliance.data.positive_behaviour_support_framework && (
                                          <div style={{ backgroundColor: 'white', padding: '12px', borderRadius: '6px', border: '1px solid #e9d5ff' }}>
                                            ...
                                            <div className="flex items-center justify-between">
                                              <span className={`badge badge-outline rounded-full ${bspAnalysisData.bsp_analysis.compliance.data.positive_behaviour_support_framework.status === 'compliant' ? 'badge-success' : 'badge-danger'}`} style={{ fontSize: '0.75rem', padding: '4px 8px' }}>
                                        ...
                                        {bspAnalysisData.bsp_analysis.compliance.data.restrictive_practice_documentation && (
                                          <div style={{ backgroundColor: 'white', padding: '12px', borderRadius: '6px', border: '1px solid #e9d5ff' }}>
                                            ...
                                            <div className="flex items-center justify-between">
                                              <span className={`badge badge-outline rounded-full ${bspAnalysisData.bsp_analysis.compliance.data.restrictive_practice_documentation.status === 'compliant' ? 'badge-success' : 'badge-danger'}`} style={{ fontSize: '0.75rem', padding: '4px 8px' }}>
                                        ...
                                        {bspAnalysisData.bsp_analysis.compliance.data.environment_safety_obligations && (
                                          <div style={{ backgroundColor: 'white', padding: '12px', borderRadius: '6px', border: '1px solid #e9d5ff' }}>
                                            ...
                                            <div className="flex items-center justify-between">
                                              <span className={`badge badge-outline rounded-full ${bspAnalysisData.bsp_analysis.compliance.data.environment_safety_obligations.status === 'compliant' ? 'badge-success' : 'badge-danger'}`} style={{ fontSize: '0.75rem', padding: '4px 8px' }}>
                                        ...
                                        {bspAnalysisData.bsp_analysis.compliance.data.incident_documentation_standards && (
                                          <div style={{ backgroundColor: 'white', padding: '12px', borderRadius: '6px', border: '1px solid #e9d5ff' }}>
                                            ...
                                            <div className="flex items-center justify-between">
                                              <span className={`badge badge-outline rounded-full ${bspAnalysisData.bsp_analysis.compliance.data.incident_documentation_standards.status === 'compliant' ? 'badge-success' : 'badge-danger'}`} style={{ fontSize: '0.75rem', padding: '4px 8px' }}>
                    ...
                    {activeModalTab === 'details' && (
                      <div className="incident-modal-tab-content">
                        ...
                        <div style={{
                          ...
                          <div style={{ marginBottom: '16px' }}>
                            <div style={{
                              ...
                              <span className={`badge badge-outline ${getStatusBadgeClass(selectedIncidentDetails.status)} rounded-full`} style={{ fontSize: '0.6875rem', padding: '4px 10px' }}>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                                  ...
                                  <div>
                                    ...
                                    <span className={`badge badge-outline ${getSeverityBadgeClass(selectedIncidentDetails.severity)} rounded-full`} style={{ fontSize: '0.75rem', padding: '3px 10px' }}>
                            ...
                            <div style={{
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', overflow: 'hidden' }}>
                                <div>
                                  ...
                                  <span className={`badge badge-outline ${getStatusBadgeClass(selectedIncidentDetails.status)} rounded-full`} style={{ fontSize: '0.75rem', padding: '3px 10px' }}>
                                    {selectedIncidentDetails.status || 'Draft'}
                                  </span>
                                </div>
                                <div>
                                  ...
                                  <div>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                      {selectedIncidentDetails.key_contributing_factors.map((factor: string, idx: number) => (
                                        <span
                                          key={idx}
                                          className="badge badge-outline badge-primary rounded-full"
                                          style={{
                                            padding: '6px 12px',
                                            fontSize: '0.75rem',
                                            fontWeight: '500',
                        ...
                        <div style={{ marginBottom: '16px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px', paddingLeft: '4px', width: '100%' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            >
                              <i className={`ki-outline ${Object.values(collapsedSections).some(v => v) ? 'ki-plus-square' : 'ki-minus-square'}`}></i>
                              <span>{Object.values(collapsedSections).some(v => v) ? 'Expand All' : 'Collapse All'}</span>
                            </button>
                          </div>
                          ...
                          <div style={{
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                ...
                                <span className={`badge badge-outline ${selectedIncidentDetails.is_ndis_reportable ? 'badge-success' : 'badge-light'} rounded-full`} style={{ fontSize: '0.75rem', padding: '4px 10px' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                ...
                                <span className={`badge badge-outline ${selectedIncidentDetails.police_notified ? 'badge-info' : 'badge-light'} rounded-full`} style={{ fontSize: '0.75rem', padding: '4px 10px' }}>
                          ...
                          <div style={{
                            ...
                            <div className="grid md:grid-cols-2 gap-3">
                              <div>
                                ...
                                <span className={`badge badge-outline rounded-full ${selectedIncidentDetails.recurrence_likelihood === 'High' ? 'badge-danger' :
                                  selectedIncidentDetails.recurrence_likelihood === 'Medium' ? 'badge-warning' : 'badge-success'
                                  }`} style={{
                                    display: 'inline-block',
                                    fontSize: '0.75rem',
                                    fontWeight: '600',
                            ...
                            <div style={{
                              ...
                              <div style={{ marginBottom: '16px' }}>
                                <div style={{
                                  ...
                                  {participantDetails && (
                                    <span className={`badge badge-outline rounded-full ${participantDetails.status === 'active' || participantDetails.status === true ? 'badge-success' : 'badge-danger'}`} style={{ fontSize: '0.6875rem', padding: '4px 10px' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                      ...
                                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                                        ...
                                        <div>
                                          ...
                                          <p style={{ fontSize: '0.8125rem', color: '#111827', fontWeight: '500', margin: 0 }}>
                                            {participantDetails.dob ? new Date(participantDetails.dob).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : 'Not Provided'}
                                          </p>
                                        </div>
                                      </div>
                                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                                        ...
                                        <div>
                                          ...
                                          <p style={{ fontSize: '0.8125rem', color: '#111827', fontWeight: '500', margin: 0 }}>
                                            {participantDetails.gender === 'male' ? 'Male' : participantDetails.gender === 'female' ? 'Female' : participantDetails.gender === 'other' ? 'Other' : participantDetails.gender === 'prefer_not_to_say' ? 'Prefer Not To Say' : 'Not Provided'}
                                          </p>
                                        </div>
                                      </div>
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', overflow: 'hidden' }}>
                                      ...
                                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                                        ...
                                        <div>
                                          ...
                                          <p style={{ fontSize: '0.8125rem', color: '#111827', fontWeight: '500', margin: 0 }}>
                                            {participantDetails.assigned_practitioner ? `${participantDetails.assigned_practitioner.first_name} ${participantDetails.assigned_practitioner.last_name}` : 'Not Provided'}
                                          </p>
                                        </div>
                                      </div>
                                    </div>
                              ...
                              <div style={{
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                                  <div>
                                    ...
                                    <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                      {participantDetails.medical_conditions || 'Not Provided'}
                                    </p>
                                  </div>
                                  <div>
                                    ...
                                    <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                      {participantDetails.medications || 'Not Provided'}
                                    </p>
                                  </div>
                                  <div>
                                    ...
                                    <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                      {participantDetails.disabilities || 'Not Provided'}
                                    </p>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.behaviours_of_concern || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.triggers_antecedents || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.early_warning_signs || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.escalation_patterns || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.behaviour_overview_summary || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.behavioural_tendencies || 'Not Provided'}
                                  </p>
                                </div>
                              </div>
                              ...
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.proactive_strategies || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.reactive_strategies || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.escalation_steps || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.restricted_practices || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.support_requirements || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.risk_indicators || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.risk_factors || 'Not Provided'}
                                  </p>
                                </div>
                              </div>
                              ...
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.sensory_profile || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.communication_style || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.environmental_needs || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.support_environment_family_context || 'Not Provided'}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p style={{ fontSize: '0.8125rem', color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap', margin: 0 }}>
                                    {participantDetails.functional_daily_living_profile || 'Not Provided'}
                                  </p>
                                </div>
                              </div>
                            </div>
