...
const IncidentsTimeline = ({ activeView, onViewChange }: IncidentsTimelineProps) => {
  ...
  // Handle participant_id from URL
  useEffect(() => {
    const searchParams = new URLSearchParams(location.search);
    const participantIdFromUrl = searchParams.get('participant_id');

    if (participantIdFromUrl && participantIdFromUrl !== selectedParticipant) {
      setSelectedParticipant(participantIdFromUrl);
      setShowFilters(true); // Show filters if a participant ID is pre-selected
      // Trigger refresh to apply the filter
      setRefreshKey(prev => prev + 1);
    }
  }, [location.search]); // Depend on location.search to re-evaluate when URL changes
  ...
  // Load data when page or refreshKey change
  useEffect(() => {
    loadIncidents();
  ...
  const loadIncidents = async () => {
    try {
      ...
      // Add custom filters from direct UI states
      if (selectedParticipant !== 'all') {
        apiFilters.participant_id = parseInt(selectedParticipant);
      }
                        ...
                        for (let i = startPage; i <= endPage; i++) {
                          pages.push(
                            <button
                              ...
                              className={`btn btn-sm min-w-[2.5rem] ${currentPage === i
                                ? 'btn-primary'
                                : 'btn-light'
                                }`}
                            >
