...
import { setServiceLocations, type ServiceLocation as ReduxServiceLocation } from '@/redux/slices/service-offering-slice';
...
const EditMapboxLocationSelector: React.FC<EditMapboxLocationSelectorProps> = ({
  ...
  const { locations: reduxLocations } = useAppSelector((state) => state.serviceOffering);
  ...
  const addMarkerToMap = (location: ServiceLocation, markerNumber: number) => {
    if (!mapRef.current) return;

    const markerElement = createMarkerElement(markerNumber);
        ...
        setLocations((currentLocations) =>
          currentLocations.map((loc) => {
            if (loc.id === location.id) {
              ...
              createRadiusCircle(location.id, lngLat.lat, lngLat.lng, loc.radius);

              // Fetch suburbs for new location after a short delay
              setTimeout(() => {
                fetchSuburbsInRadius(location.id, lngLat.lat, lngLat.lng, loc.radius);
              }, 1000);
  ...
  const addLocationDirect = (lat: number, lng: number, address: string) => {
    ...
    setLocations((prev) => {
      ...
      if (mapRef.current) {
        ...
        addMarkerToMap(newLocation, markerNumber);
